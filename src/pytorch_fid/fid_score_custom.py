...
from conv_model import CustomCNN
...
parser.add_argument(
    ...
    type=int,
    default=512,
    help="Dimensionality of features returned by the custom CNN"
)
parser.add_argument(
    "--num-workers",
    type=int,
...
def get_activations(
    files, model, batch_size=50, dims=512, device="cpu", num_workers=1
):
    ...
    for batch in tqdm(dataloader):
        ...
        with torch.no_grad():
            pred = model(batch)
            pred = pred.cpu().numpy()  # (batch_size, 512)
...
def calculate_fid_given_paths(paths, batch_size, device, dims, num_workers=1):
    ...
    # Load your custom 512-dim model instead:
